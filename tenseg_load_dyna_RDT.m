function [w_t,dqb_t,dqb_d_t,dqb_dd_t,l0_t,E_qa_new,E_qb_new]=tenseg_load_dyna_RDT(tspan,ind_w,w,ind_dqb,dqb0,ind_l0,dl0,l0,E_qa,E_qb,gravity,acc,C,mass)
...
%initialize force 
w1=zeros(size(G,1),1); %zero external force
w1(ind_w)=w;  %force exerted on bottom nodes
w_t=w1*linspace(0,1,substep)+G*linspace(1,1,substep);
...
% dqb_t=dqb*[linspace(0,1,round(substep/2)),ones(1,substep-round(substep/2))];        % slope + flat
...
% 
% %% rest length
% dl0_i=zeros(size(l0_c));
% dl0_i(ind_dl0_c)=dl0_c;
% dl0_ct=dl0_i*linspace(0,1,substep);
% l0_ct=dl0_ct+l0_c*linspace(1,1,substep);
% 
% end
% % 
