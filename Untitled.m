...
xp=1;                %细分开合度用
rate0=linspace(0.4,0.4,xp);
p=6;          %complexity for cable dome(Outer ring node)；2的倍数
n_t0=zeros(3*7*p,xp);%7*p为点数
...
gr_num=1;      %同一个环，组数划分（1，2，3，4，6）；节点数需为其的整数倍数
    ...
    h2=0.14;       %上环高度
    h4=h3*r0/R;         %下中环高度
    h1=(((r0*cos(pi/p))^2+h4^2)^0.5)*(h2-h4)/((h2-h4)^2+(R*cos(pi/p)-r0*cos(pi/p))^2)^0.5+h4;       %上中环高度    
    ...
    N_1_0=[T1*[r0*cos(pi/p)+(R*cos(pi/p)-r0*cos(pi/p))*(h1-h4)/(h2-h4);0;h1]];       %上内初节点
    N_5_0=[T2*[r0;0;h4]];       %下内初节点
    N_2_0=[T2*[R;0;h2]];      %上初节点
    N_3_0=[T2*[R;0;h3]];      %下初节点1
    N_4_0=[T2*[R;0;h3]];      %下初节点1
    N_6_0=[T1*[R*cos(pi/p);0;h2]];    %环索主力上斜索
    N_7_0=[T1*[R*cos(pi/p);0;h3]];   %环索主力下斜索
    ...
    N=[N_3,N_4,N_1,N_2,N_5,N_6,N_7];  

    C_b_in=[];

    C_s_in=[[1:1:p]',[4*p+1:1:5*p]';[p+1:1:2*p]',[4*p+1:1:5*p]';[4*p+2:1:5*p,4*p+1]',[2*p+2:1:3*p,2*p+1]';[4*p+1:1:5*p]',[2*p+2:1:3*p,2*p+1]';[2*p+1:1:3*p]',[3*p+1:1:4*p]';
        [2*p+2:1:3*p,2*p+1]',[3*p+1:1:4*p]';[4*p+1:1:5*p]',[4*p+2:1:5*p,4*p+1]';[4*p+1:1:5*p]',[5*p+1:1:6*p]';[4*p+1:1:5*p]',[5*p+2:1:6*p,5*p+1]';
        [4*p+1:1:5*p]',[6*p+1:1:7*p]';[4*p+1:1:5*p]',[6*p+2:1:7*p,6*p+1]'];
...
pinned_X=([1:1:2*p,3*p+1:1:4*p,5*p+1:1:6*p,6*p+1:1:7*p])'; pinned_Y=([1:1:2*p,3*p+1:1:4*p,5*p+1:1:6*p,6*p+1:1:7*p])'; pinned_Z=([1:1:2*p,3*p+1:1:4*p,5*p+1:1:6*p,6*p+1:1:7*p])';
...
[gr] = cable_ring_gr_copy_experiment5(gr_num,p);
...
fd=fd1*ones(3,1);
    case 12
index_gp=[p+1:3*p,7*p+1:8*p];%六组
fd=fd1*ones(18,1);
end

%fd=[1000; 1000];                       % force in bar is given as -1000
% fd=[1000];
[t_gp,t]=tenseg_prestress_design2(Gp,l,l_gp,A_2ag,V2,w0a,index_gp,fd);    %prestress design
% t_c=pinv(S')*t;  %31
t_c=t_gp;
