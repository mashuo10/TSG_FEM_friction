...
mue=data.mue;                   % friction coefficient
...
C_end=C;
C_end(find(C==-1))=0;
n_sa=size(E_sa,2);  %number of free sliding nodes
...
while icrm<MaxIcr %&& ~data.StopCriterion(U)&& lmd<=1
    ...
    while err>tol && iter<MaxIter
        ...
        alpha=friction_factor(mue,nn,free_sld,H,C);     %friction factor
        sigma_bar=kron(eye(2),E_sa)*[diag(alpha.^-1)*C_end'-C_sta';diag(alpha.^-1)*C_sta'-C_end']*t+kron([-1;1],E_qsa'*w);
        sa=E_sa'*sld;
        sa_pos=(abs(sa)+sa)/2;        sa_neg=(abs(sa)-sa)/2;
        sa_bar=[sa_pos;sa_neg];
        Phi_s=sqrt((X1\sigma_bar).^2+(X2*sa_bar).^2+2*1e-14^2)-(X1\sigma_bar+X2*sa_bar);
        ...
        K_T_fri=[E_na'*(Kn+A_2*diag(E.*A./l0)*A_2'-A_2*diag(l.\t)*A_2')*E_na,...
            E_na'*(A_2*diag(E.*A./l0s)*C*E_sa*[eye(n_sa),-eye(n_sa)]);...
            (diag(X1\sigma_bar./Phi_s)-diag(X1.^-1))*kron(eye(2),E_sa)*[diag(alpha.^-1)*C_end'-C_sta';diag(alpha.^-1)*C_sta'-C_end']*diag(E.*A./l0s)*A_2'*E_na,...
            C'*diag(E.*A./l0s)*C];          %%%%%%% not finished
